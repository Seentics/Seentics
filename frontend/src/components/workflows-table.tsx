...
import { MoreHorizontal, Pencil, Trash2, Copy, Loader2, Eye, FileJson, Search } from 'lucide-react';
...
import { deleteWorkflow, updateWorkflow, type Workflow, getWorkflows } from '@/lib/workflow-api';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
              ...
              <TableBody>
                {demoWorkflows.map((workflow) => (
                  <TableRow key={workflow.id} className="hover:bg-muted/30 transition-colors">
                    <TableCell>
                      <Link href={`/demo/workflows/${workflow.id}`} className="font-medium hover:underline text-foreground">
                        {workflow.name}
                      </Link>
                    ...
                    <TableCell className="font-medium">{workflow.totalTriggers.toLocaleString()}</TableCell>
